...
public class AgendaDAO {
    ...
    public boolean agregarAgenda(Agenda agenda) {
        String sqlAgregar = "INSERT INTO agenda (id_agenda, dia, hora, texto, id_trabajador) VALUES (?, ?, ?, ?, ?)";
        boolean resultado = false;

        try (Connection conn = ConexionBaseDatos.getConexion();
        PreparedStatement psAgregarAgenda = conn.prepareStatement(sqlAgregar)) {
            psAgregarAgenda.setInt(1, agenda.getIdAgenda());
            psAgregarAgenda.setString(2, agenda.getDia());
            psAgregarAgenda.setString(3, agenda.getHora());
            psAgregarAgenda.setString(4, agenda.getTexto());
            psAgregarAgenda.setInt(5, agenda.getIdTrabajador());
    ...
    public boolean actualizarAgenda(Integer idAgenda, String texto) {
        String sqlActualizarAgenda = "UPDATE agenda SET texto = ? WHERE id_agenda = ?";
        boolean resultado =  false;
